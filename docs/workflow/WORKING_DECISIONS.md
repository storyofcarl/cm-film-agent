...
| WD02 | Shared nested thumbnail strip at the top of every project view, above the workspace and inspector. Preserve hierarchy and shot selection across views; open a shot to direct it. Keep dedicated Assets, Batches, and History views. | Owner corrected page-dependent placement on 2026-09-17; D08 supersedes the separate Hierarchy page and bottom-only shot strip. | Top placement confirmed by owner; remaining layout open to review |
...
| WD20 | Separate deployment `cm-film-agent-studio`, same authorized team, root `apps/studio`. | Owner explicitly approved Vercel on 2026-09-17 after the destination-specific request. Project and allowlisted environment are created; hosted verification is underway. Legacy deployment remains separate. | Vercel deployment approved |
