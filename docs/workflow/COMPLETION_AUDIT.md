...
| Project strip on every view, inside the project beside the left sidebar | D45/WD42 replace dropdowns with lower-right thumbnail drill-down icons and a leading drill-up button. The project title stays fixed above compact ID/runtime thumbnails; selection is separate from drilling. Shot details are below the image, with no middle sidebar. Browser checks cover navigation, persistent chat, version switching and below-image layout | Owner review of the implementation; checks do not substitute for acceptance |
...
| Long-film chat and retained production history | WD40 adds indexed exact-source reads with full-inventory retention and source coverage. WD41 adds immutable text records and private multipart import/download, verified with a 15.5 MB/40-version browser fixture, retained snapshots and an appended V41 | Engineering incomplete: resumable preparation beyond 12 passes, full-deliverable output partitioning, record lifecycle/upper-bound scale validation and the real long-film pilot |
