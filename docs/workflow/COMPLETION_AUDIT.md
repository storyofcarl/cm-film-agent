...
| Project strip on every view, inside the project beside the left sidebar | Breadcrumb interaction rejected (D36); stacked header also rejected (D44). One module now contains level/scope selectors, metadata, scoped progress and thumbnails (WD35). Browser checks preserve scope and selection, correct branch switching and scoped shot grid | Owner review of the replacement; checks do not substitute for acceptance |
