...
| Project strip on every view, inside the project beside the left sidebar | D46/WD44 retain all project shots grouped by scene outlines and sequence/act ranges, with arrows and wheel panning but no visible scrollbar. Rollups open grids for bulk editing; shot details remain below the image. D47 adds phase approval markers below the chat composer | Owner review of the implementation; checks do not substitute for acceptance |
