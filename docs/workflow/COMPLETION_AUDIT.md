...
| Project strip on every view, inside the project beside the left sidebar | Breadcrumb interaction rejected (D36); stacked header also rejected (D44). One module contains level/scope selectors, metadata, scoped progress and thumbnails (WD35). WD39 separates selecting a thumbnail from changing scope, keeps scope editing inline and follows the selected branch when changing levels. Browser checks preserve scope and selection, correct branch switching and scoped shot grid | Owner review of the replacement; checks do not substitute for acceptance |
...
| Long-film chat and retained production history | WD40 adds indexed exact-source reads for oversized contexts, full-inventory retention, required-source coverage and retained read audits. Mocked multi-version/full-film tests exercise this path; selected scene does not narrow scope | Engineering incomplete: resumable preparation beyond 12 passes, full-deliverable output partitioning, storage beyond the 12 MB cap and project transport within Vercel's documented 4.5 MB request/response bound, followed by real long-film validation |
...
| Chat-led writing and manual document revision | WD36 gives writing separate file homes, full text, stable document families, version approval dropdowns, original uploads, recorded source prompts and inspected-version/unsaved-edit chat context. Mocked-provider, import and authenticated browser checks pass | Director review of actual writing and revisions; synthetic checks do not prove creative output quality |
